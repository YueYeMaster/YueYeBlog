...
function playMusic() {
  audio.play().then(() => {
    ...
    isPlaying = false; // 确保播放状态为false
    // 只记录错误，不弹出alert以避免无限弹窗
    console.log('音乐播放失败，请检查音乐文件路径');
  });
}
...
function updateProgress(e) {
  ...
  // 定期保存播放位置（每3秒保存一次，避免频繁写入）
  if (isPlaying && Math.floor(currentTime) % 3 === 0 && Math.floor(currentTime) !== lastSavedTime) {
...
function restorePlayState() {
  try {
    ...
    if (savedState) {
      ...
      currentMusicIndex = state.currentMusicIndex || 0;
      isPlaying = state.isPlaying || false;
      lastPlayTime = state.lastPlayTime || 0;
      return true;
    }
  } catch (error) {
    console.warn('无法恢复播放状态:', error);
  }
  return false;
}
...
function init() {
  ...
  // 如果恢复了状态且之前在播放，尝试继续播放
  if (hasRestoredState && isPlaying) {
    loadMusic();
    // 延迟一点时间再播放，确保音频加载完成
    setTimeout(() => {
      playMusic();
    ...
    setTimeout(() => {
      ...
      alarm.classList.remove('rotating'); // 移除旋转动画
    }, 500);
  }
}
